...
#define CWISS_ATOMIC_INC(val_) atomic_fetch_add_explicit(&(val_), 1, memory_order_relaxed)
...
#if defined(__SSE2__) ||                                                             \
    (CWISS_IS_MSVCISH && (defined(_M_X64) || (defined(_M_IX86) && _M_IX86_FP >= 2)))
...
#define CWISS_PREFETCH(addr_, locality_) __builtin_prefetch(addr_, 0, locality_);
#else
...
#define CWISS_LeadingZeros(x_)                                                              \
    (CWISS_LeadingZeroes64(x_) - (uint32_t)((sizeof(unsigned long long) - sizeof(x_)) * 8))

/// Computes the number of bits necessary to represent `x_`, i.e., the bit index
/// of the most significant one.
#define CWISS_BitWidth(x_)          (((uint32_t)(sizeof(x_) * 8)) - CWISS_LeadingZeros(x_))

#define CWISS_RotateLeft(x_, bits_) (((x_) << bits_) | ((x_) >> (sizeof(x_) * 8 - bits_)))
...
static inline uint32_t CWISS_BitMask_LeadingZeros(const CWISS_BitMask *self) {
    ...
    return (uint32_t)(CWISS_LeadingZeros(self->mask << extra_bits)) >> self->shift;
}
...
static_assert((CWISS_kEmpty & CWISS_kDeleted & CWISS_kSentinel & 0x80) != 0,
              "Special markers need to have the MSB to make checking for them efficient");
static_assert(CWISS_kEmpty < CWISS_kSentinel && CWISS_kDeleted < CWISS_kSentinel,
              "CWISS_kEmpty and CWISS_kDeleted must be smaller than "
              "CWISS_kSentinel to make the SIMD test of IsEmptyOrDeleted() efficient");
static_assert(CWISS_kSentinel == -1,
              "CWISS_kSentinel must be -1 to elide loading it from memory into SIMD "
              "registers (pcmpeqd xmm, xmm)");
static_assert(CWISS_kEmpty == -128, "CWISS_kEmpty must be -128 to make the SIMD check for its "
                                    "existence efficient (psignb xmm, xmm)");
static_assert((~CWISS_kEmpty & ~CWISS_kDeleted & CWISS_kSentinel & 0x7F) != 0,
...
static_assert(CWISS_kDeleted == -2, "CWISS_kDeleted must be -2 to make the implementation of "
...
static inline CWISS_ControlByte *CWISS_EmptyGroup() {
    ...
    alignas(16) static const CWISS_ControlByte kEmptyGroup[16] = {
        CWISS_kSentinel, CWISS_kEmpty, CWISS_kEmpty, CWISS_kEmpty, CWISS_kEmpty, CWISS_kEmpty,
        CWISS_kEmpty,    CWISS_kEmpty, CWISS_kEmpty, CWISS_kEmpty, CWISS_kEmpty, CWISS_kEmpty,
...
static inline CWISS_Group CWISS_mm_cmpgt_epi8_fixed(CWISS_Group a, CWISS_Group b) {
...
static inline CWISS_BitMask CWISS_Group_Match(const CWISS_Group *self, CWISS_h2_t hash) {
    return CWISS_Group_BitMask(_mm_movemask_epi8(_mm_cmpeq_epi8(_mm_set1_epi8(hash), *self)))
}
...
static inline uint32_t CWISS_Group_CountLeadingEmptyOrDeleted(const CWISS_Group *self) {
    CWISS_Group special = _mm_set1_epi8((uint8_t)CWISS_kSentinel);
    return CWISS_TrailingZeros(
        (uint32_t)(_mm_movemask_epi8(CWISS_mm_cmpgt_epi8_fixed(special, *self)) + 1));
}

static inline void CWISS_Group_ConvertSpecialToEmptyAndFullToDeleted(const CWISS_Group *self,
...
CWISS_INLINE_NEVER static bool CWISS_ShouldInsertBackwards(size_t hash,
                                                           const CWISS_ControlByte *ctrl) {
...
CWISS_INLINE_NEVER static void CWISS_ConvertDeletedToEmptyAndFullToDeleted(CWISS_ControlByte *ctrl,
                                                                           size_t capacity) {
    CWISS_DCHECK(ctrl[capacity] == CWISS_kSentinel, "bad ctrl value at %zu: %02x", capacity,
                 ctrl[capacity]);
...
/// array as deleted.
static inline void CWISS_ResetCtrl(size_t capacity, CWISS_ControlByte *ctrl, const void *slots,
                                   size_t slot_size) {
...
static inline void CWISS_SetCtrl(size_t i, CWISS_ControlByte h, size_t capacity,
                                 CWISS_ControlByte *ctrl, const void *slots, size_t slot_size) {
    ...
    // twice.
    size_t mirrored_i =
        ((i - CWISS_NumClonedBytes()) & capacity) + (CWISS_NumClonedBytes() & capacity);
    ctrl[i]          = h;
    ctrl[mirrored_i] = h;
}
...
/// array.
static inline size_t CWISS_AllocSize(size_t capacity, size_t slot_size, size_t slot_align) {
...
static inline CWISS_ProbeSeq CWISS_ProbeSeq_Start(const CWISS_ControlByte *ctrl, size_t hash,
                                                  size_t capacity) {
...
static inline CWISS_FindInfo CWISS_FindFirstNonFull(const CWISS_ControlByte *ctrl, size_t hash,
                                                    size_t capacity) {
        ...
        if (mask.mask) {
            ...
            if (!CWISS_IsSmall(capacity) && CWISS_ShouldInsertBackwards(hash, ctrl)) {
                return (CWISS_FindInfo){
                    CWISS_ProbeSeq_offset(&seq, CWISS_BitMask_HighestBitSet(&mask)), seq.index_};
            }
#endif
            return (CWISS_FindInfo){CWISS_ProbeSeq_offset(&seq, CWISS_BitMask_TrailingZeros(&mask)),
                                    seq.index_};
        }
...
CWISS_INLINE_NEVER
static uint64_t CWISS_AbslHash_LowLevelHash(const void *data, size_t len, uint64_t seed,
    ...
    if (len > 64) {
        ...
        do {
            ...
            uint64_t cs0 = CWISS_AbslHash_LowLevelMix(chunk[0] ^ salt[1], chunk[1] ^ current_state);
            uint64_t cs1 = CWISS_AbslHash_LowLevelMix(chunk[2] ^ salt[2], chunk[3] ^ current_state);
            current_state = (cs0 ^ cs1);

            uint64_t ds0 =
                CWISS_AbslHash_LowLevelMix(chunk[4] ^ salt[3], chunk[5] ^ duplicated_state);
            uint64_t ds1 =
                CWISS_AbslHash_LowLevelMix(chunk[6] ^ salt[4], chunk[7] ^ duplicated_state);
...
static inline void CWISS_FxHash_Write(CWISS_FxHash_State *state, const void *val, size_t len) {
...
static inline void CWISS_AbslHash_Write(CWISS_AbslHash_State *state, const void *val, size_t len) {
    ...
    while (len >= CWISS_AbslHash_kPiecewiseChunkSize) {
        CWISS_AbslHash_Mix(state, CWISS_AbslHash_Hash64(val8, CWISS_AbslHash_kPiecewiseChunkSize));
...
#define CWISS_EXTRACT(needle_, default_, ...) (CWISS_EXTRACT_RAW(needle_, default_, __VA_ARGS__))
...
#define CWISS_EXTRACT_obj_copyZobj_copy           CWISS_NOTHING, CWISS_NOTHING, CWISS_NOTHING
#define CWISS_EXTRACT_obj_dtor(key_, val_)        CWISS_EXTRACT_obj_dtorZ##key_
#define CWISS_EXTRACT_obj_dtorZobj_dtor           CWISS_NOTHING, CWISS_NOTHING, CWISS_NOTHING
#define CWISS_EXTRACT_key_hash(key_, val_)        CWISS_EXTRACT_key_hashZ##key_
#define CWISS_EXTRACT_key_hashZkey_hash           CWISS_NOTHING, CWISS_NOTHING, CWISS_NOTHING
...
#define CWISS_EXTRACT_alloc_allocZalloc_alloc     CWISS_NOTHING, CWISS_NOTHING, CWISS_NOTHING
#define CWISS_EXTRACT_alloc_free(key_, val_)      CWISS_EXTRACT_alloc_freeZ##key_
#define CWISS_EXTRACT_alloc_freeZalloc_free       CWISS_NOTHING, CWISS_NOTHING, CWISS_NOTHING
#define CWISS_EXTRACT_slot_size(key_, val_)       CWISS_EXTRACT_slot_sizeZ##key_
#define CWISS_EXTRACT_slot_sizeZslot_size         CWISS_NOTHING, CWISS_NOTHING, CWISS_NOTHING
#define CWISS_EXTRACT_slot_align(key_, val_)      CWISS_EXTRACT_slot_alignZ##key_
#define CWISS_EXTRACT_slot_alignZslot_align       CWISS_NOTHING, CWISS_NOTHING, CWISS_NOTHING
#define CWISS_EXTRACT_slot_init(key_, val_)       CWISS_EXTRACT_slot_initZ##key_
#define CWISS_EXTRACT_slot_initZslot_init         CWISS_NOTHING, CWISS_NOTHING, CWISS_NOTHING
#define CWISS_EXTRACT_slot_transfer(key_, val_)   CWISS_EXTRACT_slot_transferZ##key_
#define CWISS_EXTRACT_slot_transferZslot_transfer CWISS_NOTHING, CWISS_NOTHING, CWISS_NOTHING
#define CWISS_EXTRACT_slot_get(key_, val_)        CWISS_EXTRACT_slot_getZ##key_
#define CWISS_EXTRACT_slot_getZslot_get           CWISS_NOTHING, CWISS_NOTHING, CWISS_NOTHING
#define CWISS_EXTRACT_slot_dtor(key_, val_)       CWISS_EXTRACT_slot_dtorZ##key_
#define CWISS_EXTRACT_slot_dtorZslot_dtor         CWISS_NOTHING, CWISS_NOTHING, CWISS_NOTHING
#define CWISS_EXTRACT_modifiers(key_, val_)       CWISS_EXTRACT_modifiersZ##key_
#define CWISS_EXTRACT_modifiersZmodifiers         CWISS_NOTHING, CWISS_NOTHING, CWISS_NOTHING

#define CWISS_EXTRACT00(needle_, kv_, ...)                                                         \
    CWISS_SELECT00(needle_ kv_, CWISS_EXTRACT_VALUE, kv_, CWISS_EXTRACT01, (needle_, __VA_ARGS__), \
                   CWISS_NOTHING)
#define CWISS_EXTRACT01(needle_, kv_, ...)                                                         \
    CWISS_SELECT01(needle_ kv_, CWISS_EXTRACT_VALUE, kv_, CWISS_EXTRACT02, (needle_, __VA_ARGS__), \
                   CWISS_NOTHING)
#define CWISS_EXTRACT02(needle_, kv_, ...)                                                         \
    CWISS_SELECT02(needle_ kv_, CWISS_EXTRACT_VALUE, kv_, CWISS_EXTRACT03, (needle_, __VA_ARGS__), \
                   CWISS_NOTHING)
#define CWISS_EXTRACT03(needle_, kv_, ...)                                                         \
    CWISS_SELECT03(needle_ kv_, CWISS_EXTRACT_VALUE, kv_, CWISS_EXTRACT04, (needle_, __VA_ARGS__), \
                   CWISS_NOTHING)
#define CWISS_EXTRACT04(needle_, kv_, ...)                                                         \
    CWISS_SELECT04(needle_ kv_, CWISS_EXTRACT_VALUE, kv_, CWISS_EXTRACT05, (needle_, __VA_ARGS__), \
                   CWISS_NOTHING)
#define CWISS_EXTRACT05(needle_, kv_, ...)                                                         \
    CWISS_SELECT05(needle_ kv_, CWISS_EXTRACT_VALUE, kv_, CWISS_EXTRACT06, (needle_, __VA_ARGS__), \
                   CWISS_NOTHING)
#define CWISS_EXTRACT06(needle_, kv_, ...)                                                         \
    CWISS_SELECT06(needle_ kv_, CWISS_EXTRACT_VALUE, kv_, CWISS_EXTRACT07, (needle_, __VA_ARGS__), \
                   CWISS_NOTHING)
#define CWISS_EXTRACT07(needle_, kv_, ...)                                                         \
    CWISS_SELECT07(needle_ kv_, CWISS_EXTRACT_VALUE, kv_, CWISS_EXTRACT08, (needle_, __VA_ARGS__), \
                   CWISS_NOTHING)
#define CWISS_EXTRACT08(needle_, kv_, ...)                                                         \
    CWISS_SELECT08(needle_ kv_, CWISS_EXTRACT_VALUE, kv_, CWISS_EXTRACT09, (needle_, __VA_ARGS__), \
                   CWISS_NOTHING)
#define CWISS_EXTRACT09(needle_, kv_, ...)                                                         \
    CWISS_SELECT09(needle_ kv_, CWISS_EXTRACT_VALUE, kv_, CWISS_EXTRACT0A, (needle_, __VA_ARGS__), \
                   CWISS_NOTHING)
#define CWISS_EXTRACT0A(needle_, kv_, ...)                                                         \
    CWISS_SELECT0A(needle_ kv_, CWISS_EXTRACT_VALUE, kv_, CWISS_EXTRACT0B, (needle_, __VA_ARGS__), \
                   CWISS_NOTHING)
#define CWISS_EXTRACT0B(needle_, kv_, ...)                                                         \
    CWISS_SELECT0B(needle_ kv_, CWISS_EXTRACT_VALUE, kv_, CWISS_EXTRACT0C, (needle_, __VA_ARGS__), \
                   CWISS_NOTHING)
#define CWISS_EXTRACT0C(needle_, kv_, ...)                                                         \
    CWISS_SELECT0C(needle_ kv_, CWISS_EXTRACT_VALUE, kv_, CWISS_EXTRACT0D, (needle_, __VA_ARGS__), \
                   CWISS_NOTHING)
#define CWISS_EXTRACT0D(needle_, kv_, ...)                                                         \
    CWISS_SELECT0D(needle_ kv_, CWISS_EXTRACT_VALUE, kv_, CWISS_EXTRACT0E, (needle_, __VA_ARGS__), \
                   CWISS_NOTHING)
#define CWISS_EXTRACT0E(needle_, kv_, ...)                                                         \
    CWISS_SELECT0E(needle_ kv_, CWISS_EXTRACT_VALUE, kv_, CWISS_EXTRACT0F, (needle_, __VA_ARGS__), \
                   CWISS_NOTHING)
#define CWISS_EXTRACT0F(needle_, kv_, ...)                                                         \
    CWISS_SELECT0F(needle_ kv_, CWISS_EXTRACT_VALUE, kv_, CWISS_EXTRACT10, (needle_, __VA_ARGS__), \
                   CWISS_NOTHING)
#define CWISS_EXTRACT10(needle_, kv_, ...)                                                         \
    CWISS_SELECT10(needle_ kv_, CWISS_EXTRACT_VALUE, kv_, CWISS_EXTRACT11, (needle_, __VA_ARGS__), \
                   CWISS_NOTHING)
#define CWISS_EXTRACT11(needle_, kv_, ...)                                                         \
    CWISS_SELECT11(needle_ kv_, CWISS_EXTRACT_VALUE, kv_, CWISS_EXTRACT12, (needle_, __VA_ARGS__), \
                   CWISS_NOTHING)
#define CWISS_EXTRACT12(needle_, kv_, ...)                                                         \
    CWISS_SELECT12(needle_ kv_, CWISS_EXTRACT_VALUE, kv_, CWISS_EXTRACT13, (needle_, __VA_ARGS__), \
                   CWISS_NOTHING)
#define CWISS_EXTRACT13(needle_, kv_, ...)                                                         \
    CWISS_SELECT13(needle_ kv_, CWISS_EXTRACT_VALUE, kv_, CWISS_EXTRACT14, (needle_, __VA_ARGS__), \
                   CWISS_NOTHING)
#define CWISS_EXTRACT14(needle_, kv_, ...)                                                         \
    CWISS_SELECT14(needle_ kv_, CWISS_EXTRACT_VALUE, kv_, CWISS_EXTRACT15, (needle_, __VA_ARGS__), \
                   CWISS_NOTHING)
#define CWISS_EXTRACT15(needle_, kv_, ...)                                                         \
    CWISS_SELECT15(needle_ kv_, CWISS_EXTRACT_VALUE, kv_, CWISS_EXTRACT16, (needle_, __VA_ARGS__), \
                   CWISS_NOTHING)
#define CWISS_EXTRACT16(needle_, kv_, ...)                                                         \
    CWISS_SELECT16(needle_ kv_, CWISS_EXTRACT_VALUE, kv_, CWISS_EXTRACT17, (needle_, __VA_ARGS__), \
                   CWISS_NOTHING)
#define CWISS_EXTRACT17(needle_, kv_, ...)                                                         \
    CWISS_SELECT17(needle_ kv_, CWISS_EXTRACT_VALUE, kv_, CWISS_EXTRACT18, (needle_, __VA_ARGS__), \
                   CWISS_NOTHING)
#define CWISS_EXTRACT18(needle_, kv_, ...)                                                         \
    CWISS_SELECT18(needle_ kv_, CWISS_EXTRACT_VALUE, kv_, CWISS_EXTRACT19, (needle_, __VA_ARGS__), \
                   CWISS_NOTHING)
#define CWISS_EXTRACT19(needle_, kv_, ...)                                                         \
    CWISS_SELECT19(needle_ kv_, CWISS_EXTRACT_VALUE, kv_, CWISS_EXTRACT1A, (needle_, __VA_ARGS__), \
                   CWISS_NOTHING)
#define CWISS_EXTRACT1A(needle_, kv_, ...)                                                         \
    CWISS_SELECT1A(needle_ kv_, CWISS_EXTRACT_VALUE, kv_, CWISS_EXTRACT1B, (needle_, __VA_ARGS__), \
                   CWISS_NOTHING)
#define CWISS_EXTRACT1B(needle_, kv_, ...)                                                         \
    CWISS_SELECT1B(needle_ kv_, CWISS_EXTRACT_VALUE, kv_, CWISS_EXTRACT1C, (needle_, __VA_ARGS__), \
                   CWISS_NOTHING)
#define CWISS_EXTRACT1C(needle_, kv_, ...)                                                         \
    CWISS_SELECT1C(needle_ kv_, CWISS_EXTRACT_VALUE, kv_, CWISS_EXTRACT1D, (needle_, __VA_ARGS__), \
                   CWISS_NOTHING)
#define CWISS_EXTRACT1D(needle_, kv_, ...)                                                         \
    CWISS_SELECT1D(needle_ kv_, CWISS_EXTRACT_VALUE, kv_, CWISS_EXTRACT1E, (needle_, __VA_ARGS__), \
                   CWISS_NOTHING)
#define CWISS_EXTRACT1E(needle_, kv_, ...)                                                         \
    CWISS_SELECT1E(needle_ kv_, CWISS_EXTRACT_VALUE, kv_, CWISS_EXTRACT1F, (needle_, __VA_ARGS__), \
                   CWISS_NOTHING)
#define CWISS_EXTRACT1F(needle_, kv_, ...)                                                         \
    CWISS_SELECT1F(needle_ kv_, CWISS_EXTRACT_VALUE, kv_, CWISS_EXTRACT20, (needle_, __VA_ARGS__), \
                   CWISS_NOTHING)
#define CWISS_EXTRACT20(needle_, kv_, ...)                                                         \
    CWISS_SELECT20(needle_ kv_, CWISS_EXTRACT_VALUE, kv_, CWISS_EXTRACT21, (needle_, __VA_ARGS__), \
                   CWISS_NOTHING)
#define CWISS_EXTRACT21(needle_, kv_, ...)                                                         \
    CWISS_SELECT21(needle_ kv_, CWISS_EXTRACT_VALUE, kv_, CWISS_EXTRACT22, (needle_, __VA_ARGS__), \
                   CWISS_NOTHING)
#define CWISS_EXTRACT22(needle_, kv_, ...)                                                         \
    CWISS_SELECT22(needle_ kv_, CWISS_EXTRACT_VALUE, kv_, CWISS_EXTRACT23, (needle_, __VA_ARGS__), \
                   CWISS_NOTHING)
#define CWISS_EXTRACT23(needle_, kv_, ...)                                                         \
    CWISS_SELECT23(needle_ kv_, CWISS_EXTRACT_VALUE, kv_, CWISS_EXTRACT24, (needle_, __VA_ARGS__), \
                   CWISS_NOTHING)
#define CWISS_EXTRACT24(needle_, kv_, ...)                                                         \
    CWISS_SELECT24(needle_ kv_, CWISS_EXTRACT_VALUE, kv_, CWISS_EXTRACT25, (needle_, __VA_ARGS__), \
                   CWISS_NOTHING)
#define CWISS_EXTRACT25(needle_, kv_, ...)                                                         \
    CWISS_SELECT25(needle_ kv_, CWISS_EXTRACT_VALUE, kv_, CWISS_EXTRACT26, (needle_, __VA_ARGS__), \
                   CWISS_NOTHING)
#define CWISS_EXTRACT26(needle_, kv_, ...)                                                         \
    CWISS_SELECT26(needle_ kv_, CWISS_EXTRACT_VALUE, kv_, CWISS_EXTRACT27, (needle_, __VA_ARGS__), \
                   CWISS_NOTHING)
#define CWISS_EXTRACT27(needle_, kv_, ...)                                                         \
    CWISS_SELECT27(needle_ kv_, CWISS_EXTRACT_VALUE, kv_, CWISS_EXTRACT28, (needle_, __VA_ARGS__), \
                   CWISS_NOTHING)
#define CWISS_EXTRACT28(needle_, kv_, ...)                                                         \
    CWISS_SELECT28(needle_ kv_, CWISS_EXTRACT_VALUE, kv_, CWISS_EXTRACT29, (needle_, __VA_ARGS__), \
                   CWISS_NOTHING)
#define CWISS_EXTRACT29(needle_, kv_, ...)                                                         \
    CWISS_SELECT29(needle_ kv_, CWISS_EXTRACT_VALUE, kv_, CWISS_EXTRACT2A, (needle_, __VA_ARGS__), \
                   CWISS_NOTHING)
#define CWISS_EXTRACT2A(needle_, kv_, ...)                                                         \
    CWISS_SELECT2A(needle_ kv_, CWISS_EXTRACT_VALUE, kv_, CWISS_EXTRACT2B, (needle_, __VA_ARGS__), \
                   CWISS_NOTHING)
#define CWISS_EXTRACT2B(needle_, kv_, ...)                                                         \
    CWISS_SELECT2B(needle_ kv_, CWISS_EXTRACT_VALUE, kv_, CWISS_EXTRACT2C, (needle_, __VA_ARGS__), \
                   CWISS_NOTHING)
#define CWISS_EXTRACT2C(needle_, kv_, ...)                                                         \
    CWISS_SELECT2C(needle_ kv_, CWISS_EXTRACT_VALUE, kv_, CWISS_EXTRACT2D, (needle_, __VA_ARGS__), \
                   CWISS_NOTHING)
#define CWISS_EXTRACT2D(needle_, kv_, ...)                                                         \
    CWISS_SELECT2D(needle_ kv_, CWISS_EXTRACT_VALUE, kv_, CWISS_EXTRACT2E, (needle_, __VA_ARGS__), \
                   CWISS_NOTHING)
#define CWISS_EXTRACT2E(needle_, kv_, ...)                                                         \
    CWISS_SELECT2E(needle_ kv_, CWISS_EXTRACT_VALUE, kv_, CWISS_EXTRACT2F, (needle_, __VA_ARGS__), \
                   CWISS_NOTHING)
#define CWISS_EXTRACT2F(needle_, kv_, ...)                                                         \
    CWISS_SELECT2F(needle_ kv_, CWISS_EXTRACT_VALUE, kv_, CWISS_EXTRACT30, (needle_, __VA_ARGS__), \
                   CWISS_NOTHING)
#define CWISS_EXTRACT30(needle_, kv_, ...)                                                         \
    CWISS_SELECT30(needle_ kv_, CWISS_EXTRACT_VALUE, kv_, CWISS_EXTRACT31, (needle_, __VA_ARGS__), \
                   CWISS_NOTHING)
#define CWISS_EXTRACT31(needle_, kv_, ...)                                                         \
    CWISS_SELECT31(needle_ kv_, CWISS_EXTRACT_VALUE, kv_, CWISS_EXTRACT32, (needle_, __VA_ARGS__), \
                   CWISS_NOTHING)
#define CWISS_EXTRACT32(needle_, kv_, ...)                                                         \
    CWISS_SELECT32(needle_ kv_, CWISS_EXTRACT_VALUE, kv_, CWISS_EXTRACT33, (needle_, __VA_ARGS__), \
                   CWISS_NOTHING)
#define CWISS_EXTRACT33(needle_, kv_, ...)                                                         \
    CWISS_SELECT33(needle_ kv_, CWISS_EXTRACT_VALUE, kv_, CWISS_EXTRACT34, (needle_, __VA_ARGS__), \
                   CWISS_NOTHING)
#define CWISS_EXTRACT34(needle_, kv_, ...)                                                         \
    CWISS_SELECT34(needle_ kv_, CWISS_EXTRACT_VALUE, kv_, CWISS_EXTRACT35, (needle_, __VA_ARGS__), \
                   CWISS_NOTHING)
#define CWISS_EXTRACT35(needle_, kv_, ...)                                                         \
    CWISS_SELECT35(needle_ kv_, CWISS_EXTRACT_VALUE, kv_, CWISS_EXTRACT36, (needle_, __VA_ARGS__), \
                   CWISS_NOTHING)
#define CWISS_EXTRACT36(needle_, kv_, ...)                                                         \
    CWISS_SELECT36(needle_ kv_, CWISS_EXTRACT_VALUE, kv_, CWISS_EXTRACT37, (needle_, __VA_ARGS__), \
                   CWISS_NOTHING)
#define CWISS_EXTRACT37(needle_, kv_, ...)                                                         \
    CWISS_SELECT37(needle_ kv_, CWISS_EXTRACT_VALUE, kv_, CWISS_EXTRACT38, (needle_, __VA_ARGS__), \
                   CWISS_NOTHING)
#define CWISS_EXTRACT38(needle_, kv_, ...)                                                         \
    CWISS_SELECT38(needle_ kv_, CWISS_EXTRACT_VALUE, kv_, CWISS_EXTRACT39, (needle_, __VA_ARGS__), \
                   CWISS_NOTHING)
#define CWISS_EXTRACT39(needle_, kv_, ...)                                                         \
    CWISS_SELECT39(needle_ kv_, CWISS_EXTRACT_VALUE, kv_, CWISS_EXTRACT3A, (needle_, __VA_ARGS__), \
                   CWISS_NOTHING)
#define CWISS_EXTRACT3A(needle_, kv_, ...)                                                         \
    CWISS_SELECT3A(needle_ kv_, CWISS_EXTRACT_VALUE, kv_, CWISS_EXTRACT3B, (needle_, __VA_ARGS__), \
                   CWISS_NOTHING)
#define CWISS_EXTRACT3B(needle_, kv_, ...)                                                         \
    CWISS_SELECT3B(needle_ kv_, CWISS_EXTRACT_VALUE, kv_, CWISS_EXTRACT3C, (needle_, __VA_ARGS__), \
                   CWISS_NOTHING)
#define CWISS_EXTRACT3C(needle_, kv_, ...)                                                         \
    CWISS_SELECT3C(needle_ kv_, CWISS_EXTRACT_VALUE, kv_, CWISS_EXTRACT3D, (needle_, __VA_ARGS__), \
                   CWISS_NOTHING)
#define CWISS_EXTRACT3D(needle_, kv_, ...)                                                         \
    CWISS_SELECT3D(needle_ kv_, CWISS_EXTRACT_VALUE, kv_, CWISS_EXTRACT3E, (needle_, __VA_ARGS__), \
                   CWISS_NOTHING)
#define CWISS_EXTRACT3E(needle_, kv_, ...)                                                         \
    CWISS_SELECT3E(needle_ kv_, CWISS_EXTRACT_VALUE, kv_, CWISS_EXTRACT3F, (needle_, __VA_ARGS__), \
                   CWISS_NOTHING)
#define CWISS_EXTRACT3F(needle_, kv_, ...)                                                         \
    CWISS_SELECT3F(needle_ kv_, CWISS_EXTRACT_VALUE, kv_, CWISS_EXTRACT40, (needle_, __VA_ARGS__), \
                   CWISS_NOTHING)
...
#define CWISS_DECLARE_NODE_SET_POLICY(kPolicy_, Type_, ...)                                     \
    ...
    CWISS_DECLARE_POLICY_(kPolicy_, Type_, Type_, __VA_ARGS__, CWISS_NODE_OVERRIDES_(kPolicy_))
...
#define CWISS_DECLARE_POLICY_(kPolicy_, Type_, Key_, ...)                          \
    ...
    const CWISS_SlotPolicy kPolicy_##_SlotPolicy = {                               \
        ...
        CWISS_EXTRACT(slot_transfer, kPolicy_##_DefaultSlotTransfer, __VA_ARGS__), \
...
#define CWISS_DECLARE_NODE_FUNCTIONS_(kPolicy_, Type_, ...)                                        \
    ...
    static inline void kPolicy_##_NodeSlotInit(void *slot) {                                       \
        void *node = CWISS_EXTRACT(alloc_alloc, CWISS_DefaultMalloc, __VA_ARGS__)(sizeof(Type_),   \
                                                                                  alignof(Type_)); \
...
#define CWISS_NODE_OVERRIDES_(kPolicy_)                                                  \
    (slot_size, sizeof(void *)), (slot_align, alignof(void *)),                          \
        (slot_init, kPolicy_##_NodeSlotInit), (slot_dtor, kPolicy_##_NodeSlotDtor),      \
        (slot_transfer, kPolicy_##_NodeSlotTransfer), (slot_get, kPolicy_##_NodeSlotGet)
...
static inline void CWISS_RawTable_dump(const CWISS_Policy *policy, const CWISS_RawTable *self) {
    fprintf(stderr, "ptr: %p, len: %zu, cap: %zu, growth: %zu\n", self->ctrl_, self->size_,
            self->capacity_, self->growth_left_);
    if (self->capacity_ == 0) {
        return;
    }
...
static inline CWISS_RawIter CWISS_RawTable_iter_at(const CWISS_Policy *policy, CWISS_RawTable *self,
                                                   size_t index) {
    CWISS_RawIter iter = {
        self,
        self->ctrl_ + index,
...
static inline CWISS_RawIter CWISS_RawTable_iter(const CWISS_Policy *policy, CWISS_RawTable *self) {
...
static inline CWISS_RawIter CWISS_RawTable_citer_at(const CWISS_Policy *policy,
                                                    const CWISS_RawTable *self, size_t index) {
...
static inline void *CWISS_RawIter_get(const CWISS_Policy *policy, const CWISS_RawIter *self) {
...
static inline void *CWISS_RawIter_next(const CWISS_Policy *policy, CWISS_RawIter *self) {
...
static inline void CWISS_RawTable_EraseMetaOnly(const CWISS_Policy *policy, CWISS_RawIter it) {
    ...
    CWISS_SetCtrl(index, was_never_full ? CWISS_kEmpty : CWISS_kDeleted, it.set_->capacity_,
                  it.set_->ctrl_, it.set_->slots_, policy->slot->size);
...
static inline void CWISS_RawTable_InitializeSlots(const CWISS_Policy *policy,
    ...
    CWISS_ResetCtrl(self->capacity_, self->ctrl_, self->slots_, policy->slot->size);
...
static inline void CWISS_RawTable_DestroySlots(const CWISS_Policy *policy, CWISS_RawTable *self) {
    if (!self->capacity_)
        return;
...
static inline void CWISS_RawTable_Resize(const CWISS_Policy *policy, CWISS_RawTable *self,
                                         size_t new_capacity) {
    CWISS_DCHECK(CWISS_IsValidCapacity(new_capacity), "invalid capacity: %zu", new_capacity);
    ...
    for (size_t i = 0; i != old_capacity; ++i) {
        if (CWISS_IsFull(old_ctrl[i])) {
            size_t hash = policy->key->hash(policy->slot->get(old_slots + i * policy->slot->size));
            CWISS_FindInfo target = CWISS_FindFirstNonFull(self->ctrl_, hash, self->capacity_);
            size_t new_i          = target.offset;
            total_probe_length += target.probe_length;
            CWISS_SetCtrl(new_i, CWISS_H2(hash), self->capacity_, self->ctrl_, self->slots_,
                          policy->slot->size);
...
static void CWISS_RawTable_DropDeletesWithoutResize(const CWISS_Policy *policy,
                                                    CWISS_RawTable *self) {
    CWISS_DCHECK(CWISS_IsValidCapacity(self->capacity_), "invalid capacity: %zu", self->capacity_);
    CWISS_DCHECK(!CWISS_IsSmall(self->capacity_), "unexpected small capacity: %zu",
                 self->capacity_);
    // Algorithm:
    ...
    for (size_t i = 0; i != self->capacity_; ++i) {
        ...
        const CWISS_FindInfo target = CWISS_FindFirstNonFull(self->ctrl_, hash, self->capacity_);
...
#define CWISS_ProbeIndex(pos_) (((pos_ - probe_offset) & self->capacity_) / CWISS_Group_kWidth)

        // Element doesn't move.
        if (CWISS_LIKELY(CWISS_ProbeIndex(new_i) == CWISS_ProbeIndex(i))) {
            CWISS_SetCtrl(i, CWISS_H2(hash), self->capacity_, self->ctrl_, self->slots_,
                          policy->slot->size);
            continue;
        }
        if (CWISS_IsEmpty(self->ctrl_[new_i])) {
            ...
            CWISS_DCHECK(CWISS_IsDeleted(self->ctrl_[new_i]), "bad ctrl value at %zu: %02x", new_i,
                         self->ctrl_[new_i]);
            CWISS_SetCtrl(new_i, CWISS_H2(hash), self->capacity_, self->ctrl_, self->slots_,
                          policy->slot->size);
...
static inline void CWISS_RawTable_rehash_and_grow_if_necessary(const CWISS_Policy *policy,
                                                               CWISS_RawTable *self) {
    if (self->capacity_ == 0) {
...
static inline void CWISS_RawTable_Prefetch(const CWISS_Policy *policy, const CWISS_RawTable *self,
                                           const void *key) {
    (void)key;
#if CWISS_HAVE_PREFETCH
    CWISS_RawTable_PrefetchHeapBlock(policy, self);
    CWISS_ProbeSeq seq = CWISS_ProbeSeq_Start(self->ctrl_, policy->key->hash(key), self->capacity_);
...
CWISS_INLINE_NEVER
static size_t CWISS_RawTable_PrepareInsert(const CWISS_Policy *policy, CWISS_RawTable *self,
                                           size_t hash) {
    CWISS_FindInfo target = CWISS_FindFirstNonFull(self->ctrl_, hash, self->capacity_);
    if (CWISS_UNLIKELY(self->growth_left_ == 0 && !CWISS_IsDeleted(self->ctrl_[target.offset]))) {
    ...
    CWISS_SetCtrl(target.offset, CWISS_H2(hash), self->capacity_, self->ctrl_, self->slots_,
                  policy->slot->size);
...
CWISS_RawTable_FindOrPrepareInsert(const CWISS_Policy *policy, const CWISS_KeyPolicy *key_policy,
    ...
    return (CWISS_PrepareInsert){CWISS_RawTable_PrepareInsert(policy, self, hash), true};
}
...
static inline void *CWISS_RawTable_PreInsert(const CWISS_Policy *policy, CWISS_RawTable *self,
                                             size_t i) {
...
static inline CWISS_RawTable CWISS_RawTable_new(const CWISS_Policy *policy, size_t capacity) {
    CWISS_RawTable self = {
...
static inline void CWISS_RawTable_reserve(const CWISS_Policy *policy, CWISS_RawTable *self,
                                          size_t n) {
...
static inline CWISS_RawTable CWISS_RawTable_dup(const CWISS_Policy *policy,
    ...
    // during this process.
    for (CWISS_RawIter iter = CWISS_RawTable_citer(policy, self); CWISS_RawIter_get(policy, &iter);
         CWISS_RawIter_next(policy, &iter)) {
        void *v     = CWISS_RawIter_get(policy, &iter);
        size_t hash = policy->key->hash(v);

        CWISS_FindInfo target = CWISS_FindFirstNonFull(copy.ctrl_, hash, copy.capacity_);
        CWISS_SetCtrl(target.offset, CWISS_H2(hash), copy.capacity_, copy.ctrl_, copy.slots_,
                      policy->slot->size);
...
static inline void CWISS_RawTable_destroy(const CWISS_Policy *policy, CWISS_RawTable *self) {
    CWISS_RawTable_DestroySlots(policy, self);
}

/// Returns whether the table is empty.
static inline bool CWISS_RawTable_empty(const CWISS_Policy *policy, const CWISS_RawTable *self) {
    return !self->size_;
}

/// Returns the number of elements in the table.
static inline size_t CWISS_RawTable_size(const CWISS_Policy *policy, const CWISS_RawTable *self) {
    return self->size_;
}
...
static inline CWISS_Insert CWISS_RawTable_deferred_insert(const CWISS_Policy *policy,
                                                          const CWISS_KeyPolicy *key_policy,
                                                          CWISS_RawTable *self, const void *key) {
    CWISS_PrepareInsert res = CWISS_RawTable_FindOrPrepareInsert(policy, key_policy, self, key);

    if (res.inserted) {
        CWISS_RawTable_PreInsert(policy, self, res.index);
    }
    return (CWISS_Insert){CWISS_RawTable_citer_at(policy, self, res.index), res.inserted};
}
...
static inline CWISS_Insert CWISS_RawTable_insert(const CWISS_Policy *policy, CWISS_RawTable *self,
                                                 const void *val) {
    CWISS_PrepareInsert res = CWISS_RawTable_FindOrPrepareInsert(policy, policy->key, self, val);
...
static inline CWISS_RawIter CWISS_RawTable_find_hinted(const CWISS_Policy *policy,
                                                       const CWISS_KeyPolicy *key_policy,
    ...
    while (true) {
        ...
        uint32_t i;
        while (CWISS_BitMask_next(&match, &i)) {
            char *slot = self->slots_ + CWISS_ProbeSeq_offset(&seq, i) * policy->slot->size;
            if (CWISS_LIKELY(key_policy->eq(key, policy->slot->get(slot))))
                return CWISS_RawTable_citer_at(policy, self, CWISS_ProbeSeq_offset(&seq, i));
        }
        if (CWISS_LIKELY(CWISS_Group_MatchEmpty(&g).mask))
...
/// `key_policy` is a possibly heterogenous key policy for comparing `key`'s
/// type to types in the map. `key_policy` may be `&policy->key`.
static inline CWISS_RawIter CWISS_RawTable_find(const CWISS_Policy *policy,
                                                const CWISS_KeyPolicy *key_policy,
                                                const CWISS_RawTable *self, const void *key) {
    return CWISS_RawTable_find_hinted(policy, key_policy, self, key, key_policy->hash(key));
}

/// Erases the element pointed to by the given valid iterator.
/// This function will invalidate the iterator.
static inline void CWISS_RawTable_erase_at(const CWISS_Policy *policy, CWISS_RawIter it) {
...
static inline bool CWISS_RawTable_erase(const CWISS_Policy *policy,
                                        const CWISS_KeyPolicy *key_policy, CWISS_RawTable *self,
                                        const void *key) {
...
static inline void CWISS_RawTable_rehash(const CWISS_Policy *policy, CWISS_RawTable *self,
                                         size_t n) {
    ...
    size_t m = CWISS_NormalizeCapacity(n | CWISS_GrowthToLowerboundCapacity(self->size_));
...
#define CWISS_DECLARE_LOOKUP(HashSet_, Key_) CWISS_DECLARE_LOOKUP_NAMED(HashSet_, Key_, Key_)
...
#define CWISS_DECLARE_LOOKUP_NAMED(HashSet_, LookupName_, Key_)                                    \
    CWISS_BEGIN                                                                                    \
    static inline size_t HashSet_##_##LookupName_##_SyntheticHash(const void *val) {               \
        return HashSet_##_##LookupName_##_hash((const Key_ *)val);                                 \
    }                                                                                              \
    static inline bool HashSet_##_##LookupName_##_SyntheticEq(const void *a, const void *b) {      \
        return HashSet_##_##LookupName_##_eq((const Key_ *)a, (const HashSet_##_Entry *)b);        \
    }                                                                                              \
    static const CWISS_KeyPolicy HashSet_##_##LookupName_##_kPolicy = {                            \
        ...
        HashSet_##_##LookupName_##_SyntheticEq,                                                    \
                                                                                                   ...
                                                                                                   \
    static inline const CWISS_KeyPolicy *HashSet_##_##LookupName_##_policy(void) {                 \
        return &HashSet_##_##LookupName_##_kPolicy;                                                \
    }                                                                                              \
                                                                                                   \
    static inline HashSet_##_Insert HashSet_##_deferred_insert_by_##LookupName_(HashSet_ *self,    \
                                                                                const Key_ *key) { \
        CWISS_Insert ret = CWISS_RawTable_deferred_insert(                                         \
            HashSet_##_policy(), &HashSet_##_##LookupName_##_kPolicy, &self->set_, key);           \
    ...
    static inline HashSet_##_CIter HashSet_##_cfind_hinted_by_##LookupName_(                       \
        ...
        return (HashSet_##_CIter){CWISS_RawTable_find_hinted(                                      \
            HashSet_##_policy(), &HashSet_##_##LookupName_##_kPolicy, &self->set_, key, hash)};    \
    }                                                                                              \
    static inline HashSet_##_Iter HashSet_##_find_hinted_by_##LookupName_(                         \
        ...
        return (HashSet_##_Iter){CWISS_RawTable_find_hinted(                                       \
            HashSet_##_policy(), &HashSet_##_##LookupName_##_kPolicy, &self->set_, key, hash)};    \
                                                                                                   ...
                                                                                                   \
    static inline HashSet_##_CIter HashSet_##_cfind_by_##LookupName_(const HashSet_ *self,         \
                                                                     const Key_ *key) {            \
        return (HashSet_##_CIter){CWISS_RawTable_find(                                             \
            HashSet_##_policy(), &HashSet_##_##LookupName_##_kPolicy, &self->set_, key)};          \
    }                                                                                              \
    static inline HashSet_##_Iter HashSet_##_find_by_##LookupName_(HashSet_ *self,                 \
                                                                   const Key_ *key) {              \
        return (HashSet_##_Iter){CWISS_RawTable_find(                                              \
            HashSet_##_policy(), &HashSet_##_##LookupName_##_kPolicy, &self->set_, key)};          \
    }                                                                                              \
                                                                                                   \
    static inline bool HashSet_##_contains_by_##LookupName_(const HashSet_ *self,                  \
                                                            const Key_ *key) {                     \
        return CWISS_RawTable_contains(HashSet_##_policy(), &HashSet_##_##LookupName_##_kPolicy,   \
                                       &self->set_, key);                                          \
    }                                                                                              \
                                                                                                   \
    static inline bool HashSet_##_erase_by_##LookupName_(HashSet_ *self, const Key_ *key) {        \
        return CWISS_RawTable_erase(HashSet_##_policy(), &HashSet_##_##LookupName_##_kPolicy,      \
                                    &self->set_, key);                                             \
    }                                                                                              \
                                                                                                   \
    CWISS_END                                                                                      \
...
#define CWISS_DECLARE_COMMON_(HashSet_, Type_, Key_, kPolicy_)                                     \
    ...
    static inline const Type_ *HashSet_##_CIter_get(const HashSet_##_CIter *it) {                  \
    ...
    static inline HashSet_##_Insert HashSet_##_deferred_insert(HashSet_ *self, const Key_ *key) {  \
        CWISS_Insert ret =                                                                         \
            CWISS_RawTable_deferred_insert(&kPolicy_, kPolicy_.key, &self->set_, key);             \
        return (HashSet_##_Insert){{ret.iter}, ret.inserted};                                      \
    }                                                                                              \
    static inline HashSet_##_Insert HashSet_##_insert(HashSet_ *self, const Type_ *val) {          \
        CWISS_Insert ret = CWISS_RawTable_insert(&kPolicy_, &self->set_, val);                     \
        return (HashSet_##_Insert){{ret.iter}, ret.inserted};                                      \
    }                                                                                              \
                                                                                                   \
    static inline HashSet_##_CIter HashSet_##_cfind_hinted(const HashSet_ *self, const Key_ *key,  \
                                                           size_t hash) {                          \
        return (HashSet_##_CIter){                                                                 \
            CWISS_RawTable_find_hinted(&kPolicy_, kPolicy_.key, &self->set_, key, hash)};          \
    }                                                                                              \
    static inline HashSet_##_Iter HashSet_##_find_hinted(HashSet_ *self, const Key_ *key,          \
                                                         size_t hash) {                            \
        return (HashSet_##_Iter){                                                                  \
            CWISS_RawTable_find_hinted(&kPolicy_, kPolicy_.key, &self->set_, key, hash)};          \
    }                                                                                              \
    static inline HashSet_##_CIter HashSet_##_cfind(const HashSet_ *self, const Key_ *key) {       \
        return (HashSet_##_CIter){CWISS_RawTable_find(&kPolicy_, kPolicy_.key, &self->set_, key)}; \
    }                                                                                              \
    static inline HashSet_##_Iter HashSet_##_find(HashSet_ *self, const Key_ *key) {               \
        return (HashSet_##_Iter){CWISS_RawTable_find(&kPolicy_, kPolicy_.key, &self->set_, key)};  \
    }                                                                                              \
                                                                                                   \
    static inline bool HashSet_##_contains(const HashSet_ *self, const Key_ *key) {                \

...
int main(void) {
    ...
    xpc_object_t u64_4_obj    = xpc_uint64_create(0x4);
    const char *keys[]        = {"foo"};
    const xpc_object_t vals[] = {u64_4_obj};
    xpc_object_t con_send_obj = xpc_dictionary_create(keys, vals, 1);
    printf("con_send_obj obj: %p desc: '%s'\n", req, xpc_copy_description(req));
    xpc_connection_send_message_with_reply(xpc_con, con_send_obj, queue, ^(xpc_object_t o) { event_handler(o); });
    dispatch_main();
